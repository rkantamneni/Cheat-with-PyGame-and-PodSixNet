#When your project goes to shit, ya'll need to import JESUS
import pygame
import datetime
import random
		...
		while self.pressed==False:
			...
			for event in pygame.event.get():
				...
				# if the game is started
					...
					pygame.draw.line(self.screen, [0,0,0], (0,110), (self.width,110), 10) #where, color, start, end, thickness in pixels
		...
		for x in range(len(self.deck1)): #length 26
			for y in range(1,14): #13 ranks clubs
				if self.deck1[x] in ("clubs"+str(y), "spades"+str(y), "hearts"+str(y), "diamonds"+str(y)):
					...
					#self.card["cardrank"+str(y)][self.countmake['count'+str(y)]].append(self.deck1[x])#new 
