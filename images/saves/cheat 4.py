...
class Listener(ConnectionListener):
	...
	def __init__(self, host, port):
		...
		self.countmake = dict.fromkeys(['count1', 'count2', 'count3', 'count4', 'count5', 'count6', 'count7', 'count8', 'count9', 'count10', 'count11', 'count12', 'count13'], -1)
		...
		for x in range(len(self.deck1)): #length 26
			...
			for y in range(1,14): #13 ranks clubs
				if self.deck1[x] in ("clubs"+str(y), "spades"+str(y), "hearts"+str(y), "diamonds"+str(y)):
					...
					self.card["cardrank"+str(y)][self.countmake['count'+str(y)]].append(self.deck1[x])
				...
				# if the game is started
					...
					for x in range(1,14):
						for y in range(4):
							...
							if self.pick['pick'+str(x)][y]==1:
								print('hey')
								if self.countmake['count'+str(x)]==0:
									self.wid = 50
									print('hello')
								if self.countmake['count'+str(x)]==1:
									self.wid = 300
								if self.countmake['count'+str(x)]==2:
									self.wid = 550
								if self.countmake['count'+str(x)]==3:
									self.wid = 800
	...
	def mousecheck(self, number_of_cards, number):		
		...
		if number_of_cards==1:	
			if 50<mouse[0]<215 and 50<mouse[1]<290:
				if click[0]:
					print(self.cardshow11)
					if self.cardshow11==False:
						self.cardshow11 = True				
						self.pick["pick"+str(number)][0]=1
						print(self.pick['pick1'][0])
