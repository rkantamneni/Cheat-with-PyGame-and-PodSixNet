...
class Listener(ConnectionListener):
	...
	def Network_currentrank(self, data): #Current rank card which needs to be played
		if self.my_turn==True:
			self.currentrank=data['rank']+int(2)
			print("Current rank:"+ str(self.currentrank))
		elif self.callbluff==True:
					...
					if self.my_turn==True: #If it is your turn
						...
						if 0 < mouse[0] < (self.width/2) and 340 < mouse[1] <650:
							...
							if click[0]: #Call Bluff
								...
								if correct!=self.numofcardsplayedbluff: #Won Bluff: Add list to other person, keep yous
									self.turnin=[]
									self.bluffcall=True
									self.firstbluff=True
						...
						if (self.width/2) < mouse[0] <self.width and 340 < mouse[1] < 650:
						...
						# if self.bluffcall==True and self.firstbluff==True:
						# 	self.bluffcalltxt = self.waitingfnt.render('Bluff: Correct!', 1, (225,0,0))
						# 	self.screen.blit(self.bluffcalltxt,(0,280))
						# elif self.bluffcall==False and self.firstbluff==True:
						# 	self.bluffcalltxt = self.waitingfnt.render('Bluff: Wrong!', 1, (125,250,0))
						# 	self.screen.blit(self.bluffcalltxt,(0,280))

					#self.win()#Checks to see if any one has their deck empty
					pygame.display.update()
				...
				for x in range(1,14):
					for y in range(4):
						if self.pick["pick"+str(x)][y]==1:
							...
							print(self.card["cardrank"+str(x)][y])
